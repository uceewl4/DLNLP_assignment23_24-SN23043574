...
class LSTM(nn.Module):
    def __init__(
        ...
        if bidirectional:
            self.linear = nn.Linear(
                in_features=output_dim * 2, out_features=8, bias=True
            )
        else:
            self.linear = nn.Linear(in_features=output_dim, out_features=8, bias=True)
    ...
    def forward(self, x):
        x = self.embedding(x)
        out, (h_n, c_n) = self.lstm(x)  # 8,195,64
        x = self.linear(h_n)  # take the last hidden state  # 3,8,8
